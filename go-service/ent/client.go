...
import (
	...
	"github.com/polyglot-chat/go-service/ent/migrate"
...
func (c *Client) Intercept(interceptors ...Interceptor) {
	...
	c.ChannelMember.Intercept(interceptors...)
...
func (c *ChannelClient) QueryMemberships(_m *Channel) *ChannelMemberQuery {
...
// QueryAuthor queries the author edge of a ChannelMessage.
func (c *ChannelMessageClient) QueryAuthor(_m *ChannelMessage) *UserQuery {
	query := (&UserClient{config: c.config}).Query()
	query.path = func(context.Context) (fromV *sql.Selector, _ error) {
		id := _m.ID
		step := sqlgraph.NewStep(
			sqlgraph.From(channelmessage.Table, channelmessage.FieldID, id),
			sqlgraph.To(user.Table, user.FieldID),
			sqlgraph.Edge(sqlgraph.M2O, true, channelmessage.AuthorTable, channelmessage.AuthorColumn),
		)
		fromV = sqlgraph.Neighbors(_m.driver.Dialect(), step)
		return fromV, nil
	}
...
func (c *UserClient) QueryMemberships(_m *User) *ChannelMemberQuery {
	query := (&ChannelMemberClient{config: c.config}).Query()
	query.path = func(context.Context) (fromV *sql.Selector, _ error) {
		id := _m.ID
		step := sqlgraph.NewStep(
...
// QueryChannelMessages queries the channel_messages edge of a User.
func (c *UserClient) QueryChannelMessages(_m *User) *ChannelMessageQuery {
	query := (&ChannelMessageClient{config: c.config}).Query()
	query.path = func(context.Context) (fromV *sql.Selector, _ error) {
		id := _m.ID
		step := sqlgraph.NewStep(
...
type (
	hooks struct {
		Channel, ChannelMember, ChannelMessage, User []ent.Hook
	}
	inters struct {
		Channel, ChannelMember, ChannelMessage, User []ent.Interceptor
	}
)
