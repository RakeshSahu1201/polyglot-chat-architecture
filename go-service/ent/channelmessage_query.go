...
func (_q *ChannelMessageQuery) QueryAuthor() *UserQuery {
...
func (_q *ChannelMessageQuery) WithAuthor(opts ...func(*UserQuery)) *ChannelMessageQuery {
	query := (&UserClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
//		GroupBy(channelmessage.FieldChannelID).
//		Aggregate(ent.Count()).
//		Scan(ctx, &v)
...
//
//	client.ChannelMessage.Query().
//		Select(channelmessage.FieldChannelID).
//		Scan(ctx, &v)
...
func (_q *ChannelMessageQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*ChannelMessage, error) {
	...
	if query := _q.withAuthor; query != nil {
		if err := _q.loadAuthor(ctx, query, nodes, nil,
			func(n *ChannelMessage, e *User) { n.Edges.Author = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *ChannelMessageQuery) loadAuthor(ctx context.Context, query *UserQuery, nodes []*ChannelMessage, init func(*ChannelMessage), assign func(*ChannelMessage, *User)) error {
	ids := make([]uuid.UUID, 0, len(nodes))
	nodeids := make(map[uuid.UUID][]*ChannelMessage)
	for i := range nodes {
		fk := nodes[i].UserID
...
func (_q *ChannelMessageQuery) querySpec() *sqlgraph.QuerySpec {
	...
	if fields := _q.ctx.Fields; len(fields) > 0 {
		...
		if _q.withAuthor != nil {
			_spec.Node.AddColumnOnce(channelmessage.FieldUserID)
		}
	}
