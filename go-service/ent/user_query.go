...
type UserQuery struct {
	...
	withMemberships     *ChannelMemberQuery
...
func (_q *UserQuery) QueryMemberships() *ChannelMemberQuery {
...
func (_q *UserQuery) QueryChannelMessages() *ChannelMessageQuery {
	...
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		...
		step := sqlgraph.NewStep(
			...
			sqlgraph.Edge(sqlgraph.O2M, false, user.ChannelMessagesTable, user.ChannelMessagesColumn),
		)
		fromU = sqlgraph.SetNeighbors(_q.driver.Dialect(), step)
		return fromU, nil
	}
...
func (_q *UserQuery) WithMemberships(opts ...func(*ChannelMemberQuery)) *UserQuery {
	query := (&ChannelMemberClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *UserQuery) WithChannelMessages(opts ...func(*ChannelMessageQuery)) *UserQuery {
	query := (&ChannelMessageClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *UserQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*User, error) {
	...
	if query := _q.withMemberships; query != nil {
		if err := _q.loadMemberships(ctx, query, nodes,
			func(n *User) { n.Edges.Memberships = []*ChannelMember{} },
			func(n *User, e *ChannelMember) { n.Edges.Memberships = append(n.Edges.Memberships, e) }); err != nil {
			return nil, err
		}
	}
	if query := _q.withChannelMessages; query != nil {
		if err := _q.loadChannelMessages(ctx, query, nodes,
			func(n *User) { n.Edges.ChannelMessages = []*ChannelMessage{} },
			func(n *User, e *ChannelMessage) { n.Edges.ChannelMessages = append(n.Edges.ChannelMessages, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *UserQuery) loadMemberships(ctx context.Context, query *ChannelMemberQuery, nodes []*User, init func(*User), assign func(*User, *ChannelMember)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[uuid.UUID]*User)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
...
func (_q *UserQuery) loadChannelMessages(ctx context.Context, query *ChannelMessageQuery, nodes []*User, init func(*User), assign func(*User, *ChannelMessage)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[uuid.UUID]*User)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(channelmessage.FieldUserID)
	}
	query.Where(predicate.ChannelMessage(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(user.ChannelMessagesColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
